...
def _runtime_soft_refresh(reason: str = ""):
    """Soft refresh handlers/runtime caches without touching persistent scene/object data."""
    global _next_runtime_refresh_at

    _ensure_handler_unregistered()
    _ensure_handler_registered()

    # clear only ephemeral runtime caches
    ...
    # During addon enable, Blender may expose RestrictData (no bpy.data.scenes yet).
    # In that phase we still refresh runtime state, but skip scene logging safely.
    try:
    ...
    for scene in scenes:
        _debug_log(scene, "runtime_soft_refresh", reason=reason or "periodic")
...
def _timer_func():
    ...
    if _next_runtime_refresh_at > 0.0 and now >= _next_runtime_refresh_at:
        _runtime_soft_refresh(reason="periodic")
    ...
    for scene in bpy.data.scenes:
        ...
        if _scene_idle_active(scene, now):
            if int(_scene_force_follow_ticks.get(key, 0)) <= 0:
                # Wake early if cursor changed while idle (manual move/rotate or external tool).
                if not _scene_cursor_observed_changed(scene):
...
def register():
    ...
    _runtime_soft_refresh(reason="session_start")
    _next_runtime_refresh_at = time.monotonic() + _RUNTIME_REFRESH_PERIOD
