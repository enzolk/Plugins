...
def _auto_attach_tick(scene, depsgraph, source=""):
    ...
    try:
        ...
        if force_follow:
            ...
            _debug_log(scene, "force_follow_applied", object=attach_obj.name, component=s.component_type)
            return
        ...
        if obj_changed and (not user_moved) and (not user_rotated):
            ...
            if streak >= OBJ_CHANGE_CONFIRM_TICKS:
                ...
                _debug_log(scene, "follow_obj_changed", object=attach_obj.name, component=s.component_type, confirm=streak)
                return
        else:
            try:
                obj_ptr = attach_obj.as_pointer()
        ...
        _apply_attachment_to_cursor(scene, comp_point_world, comp_q, attach_obj=attach_obj)
        _obj_save_state(scene, attach_obj)
        _debug_log(scene, "follow_applied", object=attach_obj.name, component=s.component_type)
