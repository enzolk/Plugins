...
class MayaShortcutLogger(QtCore.QObject):
    ...
    def _collect_actions_from_assign_keystrings(self, key_name, mod_state, seen):
        ...
        for idx in range(1, int(count) + 1):
            ...
            if self._normalize_keystring(key_string) not in target_tokens:
                continue
    ...
    def _target_keystring_tokens(self, key_name, mod_state):
        ...
        for kv in key_variants:
            ...
            if mod_state["shift"]:
                parts.append("shift")
            parts.append(kv.lower())
            base = "+".join(parts)
            variants.add(base)
            variants.add(base + "press")
            variants.add(base + "release")

        return variants

    def _normalize_keystring(self, key_string):
        text = str(key_string).strip().lower().replace(" ", "")
        text = text.replace("control", "ctrl")
        text = text.replace("cmd", "meta")
        text = text.replace("(press)", "press")
