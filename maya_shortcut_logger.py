...
class MayaShortcutLogger(QtCore.QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._command_output_callback_id = None
        self._command_history = []
        self._capture_window_ms = 220
        self._install_command_output_callback()
    ...
    def eventFilter(self, obj, event):  # noqa: N802 (Qt API naming)
        ...
        shared_seen = []
        if app is not None:
            shared_seen = app.property("_maya_shortcut_logger_seen_shortcuts") or []

        if shortcut_str in self.logged_shortcuts or shortcut_str in shared_seen:
            self._debug("shortcut_duplicate_ignored", shortcut=shortcut_str)
            return False

        self.logged_shortcuts.add(shortcut_str)
        if app is not None:
            shared_seen = list(shared_seen)
            shared_seen.append(shortcut_str)
            app.setProperty("_maya_shortcut_logger_seen_shortcuts", shared_seen)

        try:
            possible_actions = self._collect_possible_actions(key_token, mods)
            cleaned_actions = self._clean_actions(possible_actions)
            self._debug("collection_completed", shortcut=shortcut_str, action_count=len(cleaned_actions))
            capture_start = time.time()
            QtCore.QTimer.singleShot(
                self._capture_window_ms,
                lambda s=shortcut_str, a=cleaned_actions, t=capture_start: self._finalize_shortcut_log(s, a, t),
            )
        except Exception:
    ...
    def _install_command_output_callback(self):
        if om is None:
            return
        if self._command_output_callback_id is not None:
            return
        try:
            self._command_output_callback_id = om.MCommandMessage.addCommandOutputCallback(self._on_command_output)
        except Exception:
            self._command_output_callback_id = None
    ...
    def _on_command_output(self, message, message_type=None, client_data=None):
        ...
        if not text:
            return
        # Keep only executable-like command lines.
        if not ("(" in text or text.endswith(";") or "setToolTo" in text or "dR_" in text):
            return
        self._command_history.append((time.time(), text))
        # keep short ring buffer
        if len(self._command_history) > 400:
            self._command_history = self._command_history[-400:]

    def _finalize_shortcut_log(self, shortcut_str, actions, capture_start):
        commands = self._commands_since(capture_start)
        executed = self._infer_executed_action(actions, commands)
        self._print_log_block(shortcut_str, actions, executed)

    def _commands_since(self, t0):
        return [txt for ts, txt in self._command_history if ts >= t0]

    def _infer_executed_action(self, actions, commands):
        if not commands:
            return None

        blob = self._normalize_for_match(" ".join(commands))
        best = None
        best_score = 0

        for action in actions:
            a = str(action).strip()
            if not a or a == "No action found":
                continue
            tokens = self._action_match_tokens(a)
            score = 0
            for tok in tokens:
    ...
    def _action_match_tokens(self, action):
        raw = str(action)
        base = self._normalize_for_match(raw)
        tokens = {base}

        # split camel/snake for broader matching
        split_parts = re.sub(r"([a-z])([A-Z])", r"\1 \2", raw)
        split_parts = split_parts.replace("_", " ").replace("-", " ")
        for part in split_parts.split():
            p = self._normalize_for_match(part)
            if len(p) >= 4:
                tokens.add(p)

        return sorted(tokens, key=len, reverse=True)

    def _normalize_for_match(self, txt):
        t = str(txt).lower()
    ...
    def _expand_action_details(self, name_cmd, context=None):
        ...
        # Keep concise, user-facing action names only.
        if name_cmd:
            details.append(name_cmd)
        ...
        for m in self._describe_assign_command_matches(name_cmd):
            details.extend(m)
    ...
    def _describe_assign_command_matches(self, name_cmd):
        ...
        for idx in range(1, int(count) + 1):
            ...
            vals = []
            if cmd_name:
                vals.append(cmd_name)
            if rtc:
                vals.append(rtc)
            if ann:
                vals.append(ann)
            if cmd_text:
                vals.append(cmd_text)
            if vals:
                matches.append(vals)
    ...
    def _expand_token_details(self, token):
        ...
        # token may itself be a nameCommand or runTimeCommand identifier.
        out.extend(self._expand_action_details(tok))
        if self._describe_runtime_command(tok):
            out.append(tok)
            ann = self._safe_runtime_query(tok, "annotation")
            cmd = self._safe_runtime_query(tok, "command")
            if ann:
                out.append(ann)
            if cmd:
                out.append(cmd)
    ...
    def _clean_actions(self, actions):
        ...
        for a in actions:
            ...
            if not t:
                continue
            # Remove technical/noisy lines.
            low = t.lower()
            if low.startswith("hotkeyset:") or low.startswith("context:"):
                continue
            if low.startswith("no action found"):
                continue
            if "|" in t:
                # split metadata compound lines and keep meaningful tokens.
                for part in [p.strip() for p in t.split("|")]:
                    if ":" in part:
                        part = part.split(":", 1)[1].strip()
                    if part and part.lower() not in ("true", "false") and part not in seen:
                        seen.add(part)
                        clean.append(part)
                continue
        ...
        if not clean:
            clean.append("No action found")
        return clean

    def _print_log_block(self, shortcut_str, actions, executed_action=None):
        print(f'Shortcut : "{shortcut_str}"')
        print("\nPossible actions :")
        for action in actions:
            print(f"- {action}")
...
def uninstall_shortcut_logger():
    ...
    try:
        if om is not None and _SHORTCUT_LOGGER_INSTANCE._command_output_callback_id is not None:
            om.MMessage.removeCallback(_SHORTCUT_LOGGER_INSTANCE._command_output_callback_id)
    except Exception:
        pass
