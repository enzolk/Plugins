...
class OCT_OT_OriginPositionToComponent(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def _move_origin_to_component(_obj):
            ...
            if pos is None:
                return False, "No component selection found"
...
class OCT_OT_OriginOrientationToComponent(bpy.types.Operator):
    ...
    def execute(self, context):
        def _orient_origin_to_component(obj):
            rot = _selected_component_world_orientation(context)
            if rot is None:
                return False, "No component orientation available"
            return _set_object_origin_orientation_keep_appearance(obj, rot)

        ok, msg = _apply_to_each_selected_object(context, _orient_origin_to_component)
        if not ok:
            self.report({'WARNING'}, msg)
            return {'CANCELLED'}
        return {'FINISHED'}
