...
def _is_qt_object_alive(widget):
    try:
        if widget is None:
            return False
        widget.objectName()
...
class Category(QtWidgets.QFrame):
    ...
    def toggle_event(self,event):
        ...
        QtCore.QTimer.singleShot(0, self.parent_ui._safe_deferred_reflow)
...
class ELKMinimalUI(QtWidgets.QWidget):
    def __init__(self,parent=None, instance_name=None):
        ...
        self._instance_token = time.time()
        self._timers = []
        ...
        self._timers.append(self._layout_debug_resize_timer)
        self.setMinimumSize(0, 0)
        self.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.build()
        self._apply_max_height_limit()
        self.refresh()
        QtCore.QTimer.singleShot(0, lambda: self._safe_call(lambda: self.log_layout_state("ui_open")))
        QtCore.QTimer.singleShot(60, lambda: self._safe_call(lambda: self.log_layout_state("first_build_complete")))

    def _safe_call(self, fn):
        if not _is_qt_object_alive(self) or getattr(self, "_is_closing", False):
            return
        try:
            fn()
        except RuntimeError:
    ...
    def _safe_deferred_reflow(self):
        global _ACTIVE_ELK_UI
        if not _is_qt_object_alive(self) or getattr(self, "_is_closing", False) or self is not _ACTIVE_ELK_UI:
            print("[ELK UI][LIFECYCLE] Ignored reflow from deleted/stale UI instance.")
            return
        self.reflow()

    def cleanup(self):
        self._is_closing = True
        print("[ELK UI][LIFECYCLE] Cleaning previous active UI instance...")
        for timer in getattr(self, "_timers", []):
            try:
                ...
                pass
        self._timers = []
        print("[ELK UI][LIFECYCLE] Previous timers stopped.")
    ...
    def resizeEvent(self,e):
        ...
        QtCore.QTimer.singleShot(0, self._safe_deferred_reflow)
        if AUTO_RESPONSIVE_DOCK_HEIGHT:
            workspace_name = getattr(self, "_elk_workspace_name", None)
            if workspace_name and cmds.workspaceControl(workspace_name, exists=True) and not self._is_closing:
                QtCore.QTimer.singleShot(0, lambda: self._safe_call(lambda: _apply_workspace_height(workspace_name, _calc_responsive_dock_height(self))))
    ...
    def reflow(self):
        ...
        if not _is_qt_object_alive(self) or getattr(self, "_is_closing", False) or self is not _ACTIVE_ELK_UI:
            ...
            return
        workspace_name = getattr(self, "_elk_workspace_name", None)
        if workspace_name and not cmds.workspaceControl(workspace_name, exists=True):
            print("[ELK UI][LIFECYCLE] Ignored reflow because workspaceControl no longer exists.")
            return
...
def close_existing():
    global _ACTIVE_ELK_UI
    if _ACTIVE_ELK_UI is not None:
        try:
            _ACTIVE_ELK_UI.cleanup()
        except Exception:
            pass
        _ACTIVE_ELK_UI = None
...
def show(close_existing_first=True, workspace_name=WORKSPACE_NAME, floating=False):
    ...
    global _SHOW_IN_PROGRESS, _ACTIVE_ELK_UI
    if _SHOW_IN_PROGRESS:
        print("[ELK UI][DOCK] show() ignored because another show() is already in progress.")
        return
    ...
    try:
        ...
        _ACTIVE_ELK_UI = ui
        print("[ELK UI][LIFECYCLE] Active UI instance registered.")
        ...
        QtCore.QTimer.singleShot(0, ui._safe_deferred_reflow)
        QtCore.QTimer.singleShot(100, ui._safe_deferred_reflow)
        print("[ELK UI][DOCK] Reflow after dock complete.")
        cmds.workspaceControl(workspace_name, edit=True, visible=True)
        cmds.workspaceControl(workspace_name, edit=True, restore=False)
        cmds.refresh()
