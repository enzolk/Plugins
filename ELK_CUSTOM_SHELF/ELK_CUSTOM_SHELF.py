...
def _find_visible_height_parent(workspace_widget, visible_height_hint):
    best = None
    best_delta = 10 ** 9
    for wdg in _iter_parent_chain(workspace_widget):
        try:
            h = int(wdg.height())
            if h < 20 or not wdg.isVisible():
                continue
            delta = abs(h - int(visible_height_hint))
            if delta < best_delta:
                best_delta = delta
                best = wdg
        except Exception:
...
def _unlock_workspace_widget_height(name):
    ...
    try:
        ...
        pass
    for w in _ELK_DOCK_PARENT_TARGETS.get(name, []):
        try:
...
def _apply_shelf_tab_dock(workspace_name, target_height=ELK_TARGET_DOCK_HEIGHT):
    ...
    if workspace_widget is not None:
        ...
        visible_parent = _find_visible_height_parent(workspace_widget, shelf_height)
        if visible_parent is not None:
            before_h = int(visible_parent.height())
            try:
                visible_parent.setMinimumHeight(initial_height)
                visible_parent.setMaximumHeight(initial_height)
                visible_parent.resize(visible_parent.width(), initial_height)
                visible_parent.updateGeometry()
                QtWidgets.QApplication.processEvents()
            except Exception:
                pass
            after_h = int(visible_parent.height())
            cls_name = visible_parent.metaObject().className() if visible_parent.metaObject() else visible_parent.__class__.__name__
            print("[ELK DOCK HEIGHT PARENT] applied_to={}/{} before={} after={}".format(visible_parent.objectName(), cls_name, before_h, after_h))
            _ELK_DOCK_PARENT_TARGETS.setdefault(workspace_name, []).append(visible_parent)
