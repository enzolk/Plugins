...
OPTIONVAR_DOCK_INITIAL_HEIGHT_RATIO = "ELK_UI_DockInitialHeightRatioPct"
OPTIONVAR_QUICK_FAVORITES_SHORTCUT = "ELK_UI_QuickFavoritesShortcut"
OPTIONVAR_QUICK_FAVORITES_SCALE = "ELK_UI_QuickFavoritesScalePct"
DEFAULT_QUICK_FAVORITES_SHORTCUT = "Ctrl+Alt+F"
...
def _clamp_dock_height_ratio(value):
    return max(DOCK_HEIGHT_RATIO_MIN, min(DOCK_HEIGHT_RATIO_MAX, int(value)))
...
def _workspace_control_height(name):
    try:
        return int(cmds.workspaceControl(name, q=True, height=True))
    except Exception:
        pass
...
def _iter_parent_chain(widget, max_depth=12):
    chain = []
    cur = widget
    depth = 0
...
def _log_parent_chain(prefix, widget):
    for idx, wdg in enumerate(_iter_parent_chain(widget)):
        try:
            cls_name = wdg.metaObject().className() if wdg.metaObject() else wdg.__class__.__name__
            print(
                "{} idx={} className={} objectName='{}' size={}x{} minH={} maxH={}".format(
                    prefix,
                    idx,
                    cls_name,
                    wdg.objectName(),
...
def _widget_contains_shelf_control(widget):
    shelf_widget = _workspace_widget("Shelf")
    if shelf_widget is None or widget is None:
        return False
    if widget is shelf_widget:
        return True
    try:
        return shelf_widget in widget.findChildren(QtWidgets.QWidget)
    except Exception:
        return False
...
def _apply_shelf_tab_dock(workspace_name, target_height=ELK_TARGET_DOCK_HEIGHT):
    ...
    _force_workspace_widget_height(workspace_name, initial_height)
    workspace_widget = _workspace_widget(workspace_name)
    skipped_parent_resize = False
    skipped_reason = ""
    if workspace_widget is not None:
        _log_parent_chain("[ELK DOCK HEIGHT PARENT CHAIN]", workspace_widget)
        for parent in _iter_parent_chain(workspace_widget):
            if _widget_contains_shelf_control(parent):
                skipped_parent_resize = True
                skipped_reason = "shared_with_native_shelf"
                break
        if skipped_parent_resize:
            print("[ELK DOCK HEIGHT] skipped_parent_resize=True reason={}".format(skipped_reason))
            print("[ELK DOCK HEIGHT] note=tabToControl_with_Shelf_shares_parent_container; consider separate adjacent workspaceControl for independent height.")
    workspace_height_after = _workspace_control_height(workspace_name)
    qt_widget = workspace_widget
    qt_widget_height_after = None
    if qt_widget is not None:
        try:
            qt_widget_height_after = int(qt_widget.height())
        except Exception:
            pass
    print(
        "[ELK DOCK HEIGHT] shelf_height={} ratio_pct={} computed_initial_height={} "
        "workspace_height_before={} workspace_height_after={} qt_widget_height_after={}".format(
            shelf_height,
            ratio_pct,
            initial_height,
...
def _schedule_final_shelf_dock(workspace_name, target_height=ELK_TARGET_DOCK_HEIGHT):
    ...
    QtCore.QTimer.singleShot(700, lambda: _apply_shelf_tab_dock(workspace_name, target_height))
    QtCore.QTimer.singleShot(1000, lambda: _apply_shelf_tab_dock(workspace_name, target_height))
    QtCore.QTimer.singleShot(1200, lambda: _unlock_workspace_widget_height(workspace_name))
...
def _normalize_script_source(source):
    return "mel" if (source or "").strip().lower() == "mel" else "python"

def _clean_secondary_scripts(value):
    if not isinstance(value, list):
        return []
    cleaned = []
    for idx, script in enumerate(value):
        if not isinstance(script, dict):
            continue
        label = (script.get("label") or "").strip() or "Secondary Script {}".format(idx + 1)
        command = script.get("command") or ""
        cleaned.append({
            "label": label,
            "source": _normalize_script_source(script.get("source")),
...
def _script_payload(item):
    meta = {
        ...
        "quick_favorite": bool(item.get("quick_favorite", False)),
        "secondary_scripts": _clean_secondary_scripts(item.get("secondary_scripts", [])),
    }
...
def _read_script_file(path, category):
    ...
    return {"category": category, "label": meta.get("label") or path.stem, "short_name": meta.get("short_name", ""), "tooltip": meta.get("tooltip", ""), "source": source, "command": command, "icon_svg": normalize_icon_name(meta.get("icon_svg", "")), "icon_color": meta.get("icon_color", ""), "apply_elk_ui_style": bool(meta.get("apply_elk_ui_style", False)), "quick_favorite": bool(meta.get("quick_favorite", False)), "secondary_scripts": _clean_secondary_scripts(meta.get("secondary_scripts", [])), "file_path": str(path)}
...
def _execute_script_command(cmd, source="python", label="Script", apply_elk_ui_style=False):
    cmd = cmd or ""
    if not cmd.strip():
        cmds.warning("[ELK UI] Empty command: {}".format(label or "Script")); return
    use_elk_style = bool(apply_elk_ui_style)
    ...
    try:
        if _normalize_script_source(source) == "python":
            glb={"cmds":cmds,"mel":mel,"maya":__import__("maya")}; exec(cmd, glb, glb)
        else:
            mel.eval(cmd)
    except Exception as e:
        cmds.warning("[ELK UI] Error in {}: {}".format(label or "Script", e)); traceback.print_exc()
    finally:
        if use_elk_style:
...
def run_item(item):
    _execute_script_command(
        item.get("command", "") or "",
        item.get("source") or "mel",
        item.get("label", "Tool"),
        bool(item.get("apply_elk_ui_style", False))
    )
...
class ElkSecondaryScriptRow(QtWidgets.QFrame):
    def __init__(self, script, parent=None):
        super(ElkSecondaryScriptRow, self).__init__(parent)
        self.setObjectName("SecondaryScriptRow")
        self.setAttribute(QtCore.Qt.WA_TransparentForMouseEvents, True)
        lay = QtWidgets.QHBoxLayout(self)
        lay.setContentsMargins(9, 4, 9, 4)
        lay.setSpacing(8)

        source = _normalize_script_source((script or {}).get("source"))
        icon_name = "brand-python.svg" if source == "python" else "code.svg"
        icon_color = "#ffad3b" if source == "python" else "#36d6ff"
        lay.addWidget(SvgIconWidget(icon_name, icon_color, 17), 0, QtCore.Qt.AlignVCenter)

        name = QtWidgets.QLabel((script or {}).get("label") or "Secondary Script")
        name.setObjectName("SecondaryScriptName")
        lay.addWidget(name, 1, QtCore.Qt.AlignVCenter)

        badge = QtWidgets.QLabel(source.upper())
        badge.setObjectName("SecondaryScriptBadge")
        badge.setAlignment(QtCore.Qt.AlignCenter)
        lay.addWidget(badge, 0, QtCore.Qt.AlignVCenter)
...
class ELKQuickFavoriteButton(QtWidgets.QPushButton):
    def __init__(self, item, menu_widget, parent=None):
        super(ELKQuickFavoriteButton, self).__init__((item or {}).get("label", "Tool"), parent)
        self.item = item or {}
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.RightButton:
            event.accept()
            return
        super(ELKQuickFavoriteButton, self).mousePressEvent(event)

    def mouseReleaseEvent(self, event):
        if event.button() == QtCore.Qt.RightButton:
            if self.menu_widget is not None:
                self.menu_widget._open_context_menu(self, event.pos(), self.item)
            event.accept()
            return
        super(ELKQuickFavoriteButton, self).mouseReleaseEvent(event)
...
class ELKQuickFavoritesShortcutFilter(QtCore.QObject):
    def __init__(self, ui):
        super(ELKQuickFavoritesShortcutFilter, self).__init__(ui)
        self.ui = ui
    ...
    def eventFilter(self, obj, event):
        ...
        ui = self.ui
        try:
            live = _iter_live_ui_instances()
            if not live or ui is not live[-1] or not ui.isVisible():
                return False
        except Exception:
            return False
        ...
        if isinstance(focus, editable_classes):
            return False
        shortcut = QtGui.QKeySequence(getattr(ui, "quick_favorites_shortcut", DEFAULT_QUICK_FAVORITES_SHORTCUT))
        if shortcut.isEmpty():
            return False
        event_sequence = self._event_sequence(event)
        if event_sequence.matches(shortcut) == QtGui.QKeySequence.ExactMatch:
            ui.show_quick_favorites_menu()
            event.accept()
            return True
        return False

    def _event_sequence(self, event):
        key = self._qt_value(event.key())
        modifier_only = (
            self._qt_value(QtCore.Qt.Key_Control),
            self._qt_value(QtCore.Qt.Key_Shift),
            self._qt_value(QtCore.Qt.Key_Alt),
            self._qt_value(QtCore.Qt.Key_Meta),
        )
        if key in modifier_only:
            return QtGui.QKeySequence()
        mod_value = self._qt_value(event.modifiers())
        return QtGui.QKeySequence(mod_value | key)
...
class ELKQuickFavoritesMarkingMenu(QtWidgets.QFrame):
    def __init__(self, groups, launch_callback, context_callback=None, scale_pct=100, parent=None):
        flags = QtCore.Qt.Popup | QtCore.Qt.FramelessWindowHint
        super(ELKQuickFavoritesMarkingMenu, self).__init__(parent, flags)
        self.groups = groups or []
        self.launch_callback = launch_callback
        self.context_callback = context_callback
        self.scale_pct = max(50, min(200, int(scale_pct or 100)))
        self.scale = float(self.scale_pct) / 100.0
        self.card_rects = []
        self._card_ideal_positions = {}
        self._card_indices = {}
        self.center = QtCore.QPoint(self._scaled(450), self._scaled(360))
        self.setObjectName("QuickFavoritesMarkingMenu")
        self.setAttribute(QtCore.Qt.WA_TranslucentBackground, True)
        self.setFocusPolicy(QtCore.Qt.StrongFocus)
        self.setFixedSize(self._scaled(900), self._scaled(720))
        self.setStyleSheet("""
            QFrame#QuickFavoriteCard {
                background: rgba(42, 42, 42, 232);
                border: 1px solid #505050;
                border-radius: %(card_radius)dpx;
            }
            QFrame#QuickFavoriteHeader {
                background: rgba(55, 55, 55, 190);
                border: none;
                border-bottom: 1px solid rgba(255, 255, 255, 32);
                border-top-left-radius: %(card_radius)dpx;
                border-top-right-radius: %(card_radius)dpx;
            }
            QLabel#QuickFavoriteTitle {
                color: #f0f0f0;
                font-weight: 800;
                font-size: %(title_font)dpx;
                background: transparent;
            }
            QPushButton#QuickFavoriteButton {
                color: #eeeeee;
                background: transparent;
                border: none;
                border-radius: %(button_radius)dpx;
                text-align: left;
                padding: %(button_vpad)dpx %(button_hpad)dpx;
                font-size: %(button_font)dpx;
                font-weight: 500;
            }
            QPushButton#QuickFavoriteButton:hover {
                background: #505050;
            }
            QLabel#QuickFavoriteEmptyIcon {
                color: #b7b7b7;
                background: transparent;
            }
        """ % {
            "card_radius": self._scaled(9),
            "title_font": self._scaled(14),
            "button_radius": self._scaled(5),
            "button_vpad": self._scaled(4),
            "button_hpad": self._scaled(8),
            "button_font": self._scaled(12),
        })
        self._build_cards()
    ...
    def _build_cards(self):
        ...
        for index, group in enumerate(self.groups):
            card = self._build_category_card(group)
            card.setParent(self)
            card.adjustSize()
            card.resize(self._scaled(220), max(self._scaled(72), min(self._scaled(260), card.sizeHint().height())))
            ideal_pos = self._card_position(index, card.size())
            self._card_ideal_positions[card] = ideal_pos
            self._card_indices[card] = index
            card.move(ideal_pos)
            card.show()
            cards.append((index, card))
        self._apply_compact_card_layout(cards)
        self.card_rects = [card.geometry() for _index, card in cards]

    def _build_category_card(self, group):
        card = QtWidgets.QFrame()
        card.setObjectName("QuickFavoriteCard")
        lay = QtWidgets.QVBoxLayout(card)
        lay.setContentsMargins(0, 0, 0, self._scaled(8))
        lay.setSpacing(self._scaled(4))
        header = QtWidgets.QFrame(card)
        header.setObjectName("QuickFavoriteHeader")
        header_lay = QtWidgets.QHBoxLayout(header)
        header_lay.setContentsMargins(self._scaled(14), self._scaled(8), self._scaled(12), self._scaled(8))
        header_lay.setSpacing(self._scaled(8))
        icon_name = group.get("icon_svg", "")
        icon_color = group.get("icon_color", "#36d6ff")
        if icon_name:
            icon = SvgIconWidget(icon_name, icon_color, self._scaled(18))
        else:
            icon = VectorIcon(group.get("category", "tool"), icon_color, self._scaled(18))
        header_lay.addWidget(icon, 0, QtCore.Qt.AlignVCenter)
        title = QtWidgets.QLabel((group.get("category") or "Category").upper())
        title.setObjectName("QuickFavoriteTitle")
        header_lay.addWidget(title, 1, QtCore.Qt.AlignVCenter)
        lay.addWidget(header)

        for item in group.get("items", []):
            row = ELKQuickFavoriteButton(item, self)
            row.setObjectName("QuickFavoriteButton")
            row.setProperty("elkQuickFavoriteItem", item)
            row.setCursor(QtCore.Qt.PointingHandCursor)
            row.setMinimumHeight(self._scaled(28))
            row.setIcon(self._item_icon(item))
            row.setIconSize(QtCore.QSize(self._scaled(18), self._scaled(18)))
            row.clicked.connect(lambda _checked=False, fav=item: self._launch(fav))
            lay.addWidget(row)
        return card

    def _item_icon(self, item):
        path = resolve_icon_path((item or {}).get("icon_svg", ""))
        pm_size = self._scaled(22)
        render_margin = self._scaled(2)
        render_side = max(1, pm_size - (render_margin * 2))
        if path:
            renderer = QtSvg.QSvgRenderer(path.as_posix())
            if renderer.isValid():
                pm = QtGui.QPixmap(pm_size, pm_size)
                pm.fill(QtCore.Qt.transparent)
                painter = QtGui.QPainter(pm)
                renderer.render(painter, QtCore.QRectF(render_margin, render_margin, render_side, render_side))
                painter.setCompositionMode(QtGui.QPainter.CompositionMode_SourceIn)
                painter.fillRect(pm.rect(), QtGui.QColor(item_color(item)))
                painter.end()
                return QtGui.QIcon(pm)
        color = QtGui.QColor(item_color(item))
        pm = QtGui.QPixmap(pm_size, pm_size)
        pm.fill(QtCore.Qt.transparent)
        painter = QtGui.QPainter(pm)
        painter.setRenderHint(QtGui.QPainter.Antialiasing, True)
        painter.setPen(QtGui.QPen(color, self._scaled(2)))
        painter.setBrush(QtCore.Qt.NoBrush)
        painter.drawRoundedRect(QtCore.QRectF(self._scaled(5), self._scaled(5), self._scaled(12), self._scaled(12)), self._scaled(3), self._scaled(3))
        painter.drawLine(self._scaled(7), self._scaled(11), self._scaled(15), self._scaled(11))
        painter.end()
        return QtGui.QIcon(pm)

    def _card_position(self, index, size):
        slot = index % 4
        band = index // 4
        card_w = size.width()
        card_h = size.height()
        cx = self.center.x()
        cy = self.center.y()
        side_gap = self._scaled(48)
        vertical_gap = self._scaled(24)
        cascade_gap = self._scaled(10)
        if slot == 0:
            return QtCore.QPoint(cx + side_gap, cy - card_h // 2 + band * (card_h + cascade_gap))
        if slot == 1:
            return QtCore.QPoint(cx - side_gap - card_w, cy - card_h // 2 + band * (card_h + cascade_gap))
        if slot == 2:
            return QtCore.QPoint(cx - card_w // 2 + band * (card_w + cascade_gap), cy - vertical_gap - card_h)
        if slot == 3 and band == 0 and len(self.groups) > 4:
            return QtCore.QPoint(cx - card_w - self._scaled(34), cy + vertical_gap + self._scaled(18))
        return QtCore.QPoint(cx - card_w // 2 + band * (card_w + cascade_gap), cy + vertical_gap)

    def _apply_compact_card_layout(self, cards):
        gap = self._scaled(10)
        side_gap = self._scaled(48)
        top_gap = self._scaled(24)
        bottom_gap = self._scaled(42)
        by_slot = {0: [], 1: [], 2: [], 3: []}
        for index, card in cards:
            by_slot[index % 4].append((index, card))

        def stack_side(slot, right_side=True):
            entries = by_slot.get(slot, [])
            if not entries:
                return
            x_for = (
                lambda card: self.center.x() + side_gap
            ) if right_side else (
                lambda card: self.center.x() - side_gap - card.width()
            )
            y = self.center.y() - entries[0][1].height() // 2
            for _index, card in entries:
                card.move(self._clamped_card_pos(QtCore.QPoint(x_for(card), y), card.size()))
                y += card.height() + gap

        stack_side(0, True)
        stack_side(1, False)

        side_rects = [card.geometry().adjusted(-gap, -gap, gap, gap) for _index, card in by_slot.get(0, [])]
        side_rects += [card.geometry().adjusted(-gap, -gap, gap, gap) for _index, card in by_slot.get(1, [])]
        side_top_limit = min([rect.top() for rect in side_rects], default=self.center.y())
        for index, card in by_slot.get(2, []):
            band = index // 4
            ideal_y = self.center.y() - top_gap - card.height()
            clear_y = side_top_limit - gap - card.height()
            pos = QtCore.QPoint(
                self.center.x() - card.width() // 2 + band * (card.width() + gap),
                min(ideal_y, clear_y)
            )
            card.move(self._clamped_card_pos(pos, card.size()))

        left_rects = [card.geometry().adjusted(-gap, -gap, gap, gap) for _index, card in by_slot.get(1, [])]
        right_rects = [card.geometry().adjusted(-gap, -gap, gap, gap) for _index, card in by_slot.get(0, [])]
        bottom_y = self.center.y() + bottom_gap
        for index, card in by_slot.get(3, []):
            band = index // 4
            if len(self.groups) > 4:
                x = self.center.x() - card.width() - self._scaled(34) + band * (card.width() + gap)
            else:
                x = self.center.x() - card.width() // 2 + band * (card.width() + gap)
            pos = self._clamped_card_pos(QtCore.QPoint(x, bottom_y), card.size())
            probe = QtCore.QRect(pos, card.size()).adjusted(-gap, -gap, gap, gap)
            for rect in left_rects + right_rects:
                if probe.intersects(rect):
                    pos.setY(rect.bottom() + gap)
                    probe = QtCore.QRect(pos, card.size()).adjusted(-gap, -gap, gap, gap)
            card.move(self._clamped_card_pos(pos, card.size()))

    def _resolve_card_overlaps(self, cards):
        padding = 10
        for _attempt in range(32):
            pair = self._first_collision_pair(cards, padding)
            if pair is None:
                break
            first_card, second_card = pair
            move = self._best_collision_move(cards, first_card, second_card, padding)
            if move is None:
                idx = self._card_indices.get(second_card, 0)
                current = second_card.geometry().adjusted(-padding, -padding, padding, padding)
                collision = first_card.geometry().adjusted(-padding, -padding, padding, padding)
                move = (second_card, self._clamped_card_pos(second_card.pos() + self._fallback_delta(idx, current, collision), second_card.size()))
            move[0].move(move[1])

    def _first_collision_pair(self, cards, padding):
        for i, (_idx_a, card_a) in enumerate(cards):
            rect_a = card_a.geometry().adjusted(-padding, -padding, padding, padding)
            for _idx_b, card_b in cards[i + 1:]:
                rect_b = card_b.geometry().adjusted(-padding, -padding, padding, padding)
                if rect_a.intersects(rect_b):
                    return card_a, card_b
        return None

    def _best_collision_move(self, cards, first_card, second_card, padding):
        best = None
        for card in (first_card, second_card):
            index = self._card_indices.get(card, 0)
            ideal = self._card_ideal_positions.get(card, card.pos())
            for pos in self._candidate_positions(index, card.size(), ideal):
                if pos == card.pos():
                    continue
                if self._position_collides(card, pos, card.size(), cards, padding):
                    continue
                cost = self._position_cost(index, pos, card.size(), ideal)
                if best is None or cost < best[0]:
                    best = (cost, card, pos)
        return (best[1], best[2]) if best else None

    def _position_collides(self, target_card, pos, size, cards, padding):
        rect = QtCore.QRect(pos, size).adjusted(-padding, -padding, padding, padding)
        for _idx, card in cards:
            if card is target_card:
                continue
            other = card.geometry().adjusted(-padding, -padding, padding, padding)
            if rect.intersects(other):
                return True
        return False

    def _candidate_positions(self, index, size, ideal):
        slot = index % 4
        card_w = size.width()
        card_h = size.height()
        gap = 14
        ...
        for dx, dy in offsets:
            candidates.append(self._clamped_card_pos(QtCore.QPoint(ideal.x() + dx, ideal.y() + dy), size))
        return candidates

    def _position_cost(self, index, pos, size, ideal):
        slot = index % 4
        band = index // 4
        dx = pos.x() - ideal.x()
        dy = pos.y() - ideal.y()
        cost = (dx * dx) + (dy * dy)
        if band > 0 and slot == 0 and dx > 0:
            cost += 120000
        elif band > 0 and slot == 1 and dx < 0:
            cost += 120000
        elif band > 0 and slot == 2 and dy < 0:
            cost += 120000
        elif band > 0 and slot == 3 and dy > 0:
            cost += 120000
        if slot in (2, 3):
            cost += int(abs(dx) * 0.4)
        else:
            cost += int(abs(dy) * 0.4)
        center_offset = QtCore.QPoint(pos.x() + size.width() // 2, pos.y() + size.height() // 2) - self.center
        cost += int(center_offset.manhattanLength() * 3)
        return cost

    def _fallback_delta(self, index, current, collision):
        slot = index % 4
        band = index // 4
        overlap_w = max(0, min(current.right(), collision.right()) - max(current.left(), collision.left()))
        overlap_h = max(0, min(current.bottom(), collision.bottom()) - max(current.top(), collision.top()))
        step_x = max(12, overlap_w + 12)
        step_y = max(12, overlap_h + 12)
        if slot == 0:
            if band > 0:
                return QtCore.QPoint(0, step_y if current.center().y() >= self.center.y() else -step_y)
            return QtCore.QPoint(step_x, 0)
        if slot == 1:
            if band > 0:
                return QtCore.QPoint(0, step_y if current.center().y() >= self.center.y() else -step_y)
            return QtCore.QPoint(-step_x, 0)
        if slot == 2:
            if band > 0:
                return QtCore.QPoint(step_x if current.center().x() >= self.center.x() else -step_x, 0)
            return QtCore.QPoint(0, -step_y)
        if band > 0:
            return QtCore.QPoint(step_x if current.center().x() >= self.center.x() else -step_x, 0)
        return QtCore.QPoint(0, step_y)

    def _clamped_card_pos(self, pos, size):
        margin = 8
        return QtCore.QPoint(
            max(margin, min(pos.x(), self.width() - size.width() - margin)),
            max(margin, min(pos.y(), self.height() - size.height() - margin)),
        )

    def _launch(self, item):
        self.close()
        if self.launch_callback:
            self.launch_callback(item)

    def _open_context_menu(self, button, local_pos, item):
        if self.context_callback:
            self.context_callback(item, button.mapToGlobal(local_pos))

    def show_at(self, global_pos):
        screen = QtWidgets.QApplication.screenAt(global_pos) if hasattr(QtWidgets.QApplication, "screenAt") else None
        if screen is None:
            screen = QtWidgets.QApplication.primaryScreen()
        top_left = global_pos - self.center
        if screen is not None:
            geo = screen.availableGeometry()
            top_left.setX(max(geo.left(), min(top_left.x(), geo.right() - self.width())))
            top_left.setY(max(geo.top(), min(top_left.y(), geo.bottom() - self.height())))
        self.move(top_left)
        self.show()
        self.raise_()
        self.activateWindow()
    ...
    def keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Escape:
            ...
            event.accept()
            return
        super(ELKQuickFavoritesMarkingMenu, self).keyPressEvent(event)

    def mousePressEvent(self, event):
        if not any(rect.contains(event.pos()) for rect in self.card_rects):
            self.close()
            event.accept()
            return
        super(ELKQuickFavoritesMarkingMenu, self).mousePressEvent(event)
    ...
    def paintEvent(self, event):
        ...
        for rect in self.card_rects:
            painter.drawLine(self.center, rect.center())
        painter.setPen(QtGui.QPen(QtGui.QColor("#ff9f2e"), 1.4))
        painter.setBrush(QtGui.QColor(42, 42, 42, 208))
        painter.drawEllipse(self.center, self._scaled(14), self._scaled(14))
        painter.setPen(QtGui.QPen(QtGui.QColor(255, 159, 46, 44), self._scaled(6)))
        painter.drawEllipse(self.center, self._scaled(18), self._scaled(18))
        painter.end()
...
class ELKMinimalUI(QtWidgets.QWidget):
    def __init__(self,parent=None, instance_name=None):
        ...
        self.dock_initial_height_ratio_pct = self._load_dock_initial_height_ratio_pct()
        self.ui_scale_settings = self._load_ui_scale_settings()
        self.layout_debug_logs_enabled = self._load_layout_debug_logs_enabled()
        self.quick_favorites_shortcut = self._load_quick_favorites_shortcut()
        self.quick_favorites_scale_pct = self._load_quick_favorites_scale_pct()
        ...
        self._quick_favorites_shortcut_filter = ELKQuickFavoritesShortcutFilter(self)
        app = QtWidgets.QApplication.instance()
        if app is not None:
            app.installEventFilter(self._quick_favorites_shortcut_filter)
    ...
    def _quick_favorites_groups(self):
        ...
        for item in self.shelf_items:
            if not bool((item or {}).get("quick_favorite", False)):
                continue
            category = (item.get("category") or "Tools").strip() or "Tools"
            if category not in groups_by_category:
                slug = _display_to_slug(category)
                groups_by_category[category] = {
                    "category": category,
                    "icon_svg": normalize_icon_name((cat_meta.get("icons") or {}).get(slug, "")),
                    "icon_color": (cat_meta.get("icon_colors") or {}).get(slug, "#36d6ff") or "#36d6ff",
                    "items": [],
                }
                ordered_categories.append(category)
            groups_by_category[category]["items"].append(item)
    ...
    def show_quick_favorites_menu(self):
        ...
        if not groups:
            try:
                cmds.inViewMessage(amg="Aucun <hl>Quick Favorite</hl> configuré.", pos="midCenter", fade=True)
            except Exception:
                cmds.warning("[ELK UI] No Quick Favorites configured.")
            return
        try:
            if self._quick_favorites_menu is not None:
                self._quick_favorites_menu.close()
        except Exception:
            pass
        self._quick_favorites_menu = ELKQuickFavoritesMarkingMenu(
            groups,
            run_item,
            context_callback=self.open_script_context_menu,
            scale_pct=getattr(self, "quick_favorites_scale_pct", 100),
            parent=self
        )
        self._quick_favorites_menu.destroyed.connect(lambda *_: setattr(self, "_quick_favorites_menu", None))
        self._quick_favorites_menu.show_at(QtGui.QCursor.pos())
    ...
    def _load_dock_initial_height_ratio_pct(self):
        return _load_dock_initial_height_ratio_percent()
    ...
    def _load_quick_favorites_shortcut(self):
        if cmds.optionVar(exists=OPTIONVAR_QUICK_FAVORITES_SHORTCUT):
            try:
                value = str(cmds.optionVar(q=OPTIONVAR_QUICK_FAVORITES_SHORTCUT) or "").strip()
                return value or DEFAULT_QUICK_FAVORITES_SHORTCUT
            except Exception:
                pass
        return DEFAULT_QUICK_FAVORITES_SHORTCUT

    def _set_quick_favorites_shortcut(self, value):
        shortcut = (value or "").strip() or DEFAULT_QUICK_FAVORITES_SHORTCUT
        if not QtGui.QKeySequence(shortcut).isEmpty():
            self.quick_favorites_shortcut = shortcut
        else:
            self.quick_favorites_shortcut = DEFAULT_QUICK_FAVORITES_SHORTCUT
        try:
            cmds.optionVar(sv=(OPTIONVAR_QUICK_FAVORITES_SHORTCUT, self.quick_favorites_shortcut))
        except Exception:
            pass
        for ui in _iter_live_ui_instances():
            try:
                ui.quick_favorites_shortcut = self.quick_favorites_shortcut
            except Exception:
                continue

    def _load_quick_favorites_scale_pct(self):
        if cmds.optionVar(exists=OPTIONVAR_QUICK_FAVORITES_SCALE):
            try:
                return max(50, min(200, int(cmds.optionVar(q=OPTIONVAR_QUICK_FAVORITES_SCALE))))
            except Exception:
                pass
        return 100

    def _set_quick_favorites_scale_pct(self, value):
        scale = max(50, min(200, int(value)))
        self.quick_favorites_scale_pct = scale
        try:
            cmds.optionVar(iv=(OPTIONVAR_QUICK_FAVORITES_SCALE, scale))
        except Exception:
            pass
        for ui in _iter_live_ui_instances():
            try:
                ui.quick_favorites_scale_pct = scale
            except Exception:
                continue
    ...
    def _sync_horizontal_height_constraints(self):
        ...
        for cat in self.category_widgets:
            ...
            if hasattr(cat, "grid") and cat.grid is not None:
                if body_h <= 28:
                    cat.grid.setContentsMargins(2, 0, 2, 0)
                    cat.grid.setSpacing(1)
                elif body_h <= 40:
            ...
            if cat.body is not None:
                cat.body.setMinimumHeight(body_h)
                cat.body.setMaximumHeight(body_h)
    ...
    def _open_secondary_script_editor(self, parent_dialog, script=None):
        script = dict(script or {})
        dlg = QtWidgets.QDialog(parent_dialog)
        dlg.setWindowTitle("Secondary Script")
        ...
        name = QtWidgets.QLineEdit(script.get("label", ""))
        name.setPlaceholderText("e.g. Cleanup")
        source = QtWidgets.QComboBox()
        source.addItems(["python", "mel"])
        source.setCurrentText(_normalize_script_source(script.get("source")))
        source.setMaximumWidth(220)
        ...
        code_frame, code = self._build_script_editor(script.get("command", ""))
        code_frame.setMinimumHeight(210)
        ...
        def accept():
            ...
            command = code.toPlainText()
            if not label:
                show_error("Le nom du secondary script est obligatoire.")
                return
            if not command.strip():
                show_error("Le script ne peut pas être vide.")
                return
            result.update({
                "label": label,
                "source": _normalize_script_source(source.currentText()),
                "command": command,
            })
            dlg.accept()
        ...
        self._install_secondary_dialog_responsiveness(
            ...
            root,
            form_layouts=[form],
            compact_widgets=[(code_frame, 210, 150, 90)]
        )
        if dlg.exec_() if hasattr(dlg, "exec_") else dlg.exec():
            return result
        return None

    def _build_secondary_scripts_section(self, parent_dialog, secondary_scripts):
        scripts = secondary_scripts
        group = QtWidgets.QGroupBox("Secondary Scripts")
        group.setObjectName("SecondaryScriptsBox")
        lay = QtWidgets.QVBoxLayout(group)
        lay.setContentsMargins(8, 11, 8, 8)
        lay.setSpacing(8)

        script_list = QtWidgets.QListWidget()
        script_list.setObjectName("SecondaryScriptsList")
        script_list.setMinimumHeight(96)
        script_list.setAlternatingRowColors(True)
        script_list.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        script_list.setFocusPolicy(QtCore.Qt.NoFocus)
        script_list.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        lay.addWidget(script_list, 1)

        buttons = []
        for label, op in (("+ Add Secondary Script", "add"), ("Edit", "edit"), ("Remove", "remove"), ("↑", "up"), ("↓", "down")):
            ...
            buttons.append(btn)
        button_row = ElkResponsiveButtonRow(buttons, min_button_width=76)
        lay.addWidget(button_row)

        def reload_list(select_index=None):
            script_list.clear()
            for idx, script in enumerate(scripts):
                item = QtWidgets.QListWidgetItem()
                item.setData(QtCore.Qt.UserRole, idx)
                item.setSizeHint(QtCore.QSize(0, 29))
                script_list.addItem(item)
                script_list.setItemWidget(item, ElkSecondaryScriptRow(script))
            if scripts:
                target = 0 if select_index is None else max(0, min(len(scripts) - 1, int(select_index)))
                script_list.setCurrentRow(target)

        def current_index():
            row = script_list.currentRow()
            return row if 0 <= row < len(scripts) else None

        def add_script():
            edited = self._open_secondary_script_editor(parent_dialog)
            if not edited:
                return
            scripts.append(edited)
            reload_list(len(scripts) - 1)

        def edit_script():
            idx = current_index()
            if idx is None:
                return
            edited = self._open_secondary_script_editor(parent_dialog, scripts[idx])
            if not edited:
                return
            scripts[idx] = edited
            reload_list(idx)
        ...
        def remove_script():
            ...
            if idx is None:
                return
            scripts.pop(idx)
            reload_list(max(0, idx - 1))

        def move_script(offset):
            idx = current_index()
            if idx is None:
                return
            new_idx = idx + offset
            if new_idx < 0 or new_idx >= len(scripts):
                return
            scripts[idx], scripts[new_idx] = scripts[new_idx], scripts[idx]
            reload_list(new_idx)

        buttons[0].clicked.connect(add_script)
        buttons[1].clicked.connect(edit_script)
        buttons[2].clicked.connect(remove_script)
        buttons[3].clicked.connect(lambda: move_script(-1))
        buttons[4].clicked.connect(lambda: move_script(1))
        script_list.itemDoubleClicked.connect(lambda _item: edit_script())
        reload_list()
        return group, script_list, button_row
    ...
    def open_options_dialog(self):
        ...
        quick_favorites_shortcut = QtWidgets.QLineEdit(self.quick_favorites_shortcut)
        quick_favorites_shortcut.setPlaceholderText(DEFAULT_QUICK_FAVORITES_SHORTCUT)
        quick_favorites_shortcut.setToolTip("Shortcut used to open the Quick Favorites Marking Menu around the mouse.")
        form.addRow("Quick favorites shortcut:", quick_favorites_shortcut)
        quick_favorites_scale = self._add_numeric_option_row(
            form,
            "Quick menu scale:",
            50,
            200,
            int(self.quick_favorites_scale_pct),
            suffix=" %"
        )
        ...
        if dlg.exec_() if hasattr(dlg, "exec_") else dlg.exec():
            ...
            self._set_quick_favorites_shortcut(quick_favorites_shortcut.text())
            self._set_quick_favorites_scale_pct(quick_favorites_scale.value())
            optionvar_map = self._ui_scale_optionvar_map()
    ...
    def open_add_script_dialog(self):
        ...
        quick_favorite = QtWidgets.QCheckBox("Quick Favorite")
        quick_favorite.setToolTip("Show this button in the Quick Favorites Marking Menu.")
        ...
        lay.addRow("Favorite", quick_favorite)
        ...
        root.addLayout(lay)
        secondary_group, _secondary_list, _secondary_buttons = self._build_secondary_scripts_section(dlg, secondary_scripts)
        root.addWidget(secondary_group, 1)
        btns=QtWidgets.QDialogButtonBox(QtWidgets.QDialogButtonBox.Ok|QtWidgets.QDialogButtonBox.Cancel)
        root.addWidget(btns)
        self._style_dialog_buttons(btns)
        ...
        if (dlg.exec_() if hasattr(dlg,'exec_') else dlg.exec()):
            item={"label":full_name.text().strip() or "New Script","short_name":short_name.text().strip(),"category":category.currentText().strip() or "Tools","tooltip":desc.text().strip(),"source":source.currentText(),"icon_svg": normalize_icon_name(icon_name.text().strip()), "icon_color": icon_color.currentText(), "quick_favorite": bool(quick_favorite.isChecked()), "apply_elk_ui_style": bool(apply_elk_ui_style.isChecked()), "secondary_scripts": _clean_secondary_scripts(secondary_scripts), "command":code.toPlainText()}
    ...
    def open_script_context_menu(self, item, global_pos):
        ...
        secondary_actions = {}
        secondary_scripts = _clean_secondary_scripts((item or {}).get("secondary_scripts", []))
        if secondary_scripts:
            menu.addSeparator()
            for script in secondary_scripts:
                label = (script.get("label") or "").strip() or "Secondary Script"
                action = menu.addAction(label)
                secondary_actions[action] = script
        ...
        elif chosen in secondary_actions:
            script = secondary_actions[chosen]
            _execute_script_command(
                script.get("command", ""),
                script.get("source", "python"),
                script.get("label", "Secondary Script"),
                bool((item or {}).get("apply_elk_ui_style", False))
            )
    ...
    def open_edit_script_dialog(self, item):
        ...
        quick_favorite = QtWidgets.QCheckBox("Quick Favorite")
        quick_favorite.setToolTip("Show this button in the Quick Favorites Marking Menu.")
        quick_favorite.setChecked(bool(item.get("quick_favorite", False)))
        apply_elk_ui_style = QtWidgets.QCheckBox("Apply ELK visual skin on launched UI")
        apply_elk_ui_style.setToolTip("Apply ELK dark styling to windows created by this script when launched from the shelf.")
        ...
        form.addRow("Source", source)
        form.addRow("Favorite", quick_favorite)
        ...
        def do_save():
            ...
            if current_path and target_path.resolve() != current_path.resolve() and target_path.exists():
                ...
                return
            updated = {"label": name, "short_name": short, "tooltip": tip, "category": cat, "source": src, "icon_svg": normalize_icon_name(icon_name.text().strip()), "icon_color": icon_color.currentText(), "quick_favorite": bool(quick_favorite.isChecked()), "apply_elk_ui_style": bool(apply_elk_ui_style.isChecked()), "secondary_scripts": _clean_secondary_scripts(secondary_scripts), "command": cmd, "file_path": str(target_path)}
            try:
    ...
    def _apply_secondary_dialog_style(self, dialog):
        ...
        dialog.setStyleSheet("""
            ...
            QListWidget#SecondaryScriptsList {
                background: #252525;
                border: 1px solid #424242;
                border-radius: 6px;
                padding: 0px;
            ...
            QListWidget#CategoryManagerList::item, QListWidget#SecondaryScriptsList::item {
                min-height: 30px;
                border: none;
                border-bottom: 1px solid rgba(255, 255, 255, 0.035);
                background: #252525;
            }
            QListWidget#CategoryManagerList::item:alternate, QListWidget#SecondaryScriptsList::item:alternate {
                background: #2c2c2c;
            }
            QListWidget#CategoryManagerList::item:selected, QListWidget#SecondaryScriptsList::item:selected {
                background: #333333;
                border-left: 2px solid #ff9f2e;
            }
            QFrame#CategoryManagerRow, QFrame#SecondaryScriptRow {
                background: transparent;
                border: none;
            }
            QLabel#CategoryManagerName, QLabel#SecondaryScriptName {
                color: #f0f0f0;
                font-weight: 650;
                background: transparent;
                border: none;
            }
            QLabel#SecondaryScriptBadge {
                color: #d8d8d8;
                background: #373737;
                border: 1px solid #505050;
                border-radius: 5px;
            ...
            QGroupBox#SecondaryScriptsBox QPushButton {
                min-width: 0px;
                border-radius: 7px;
                background: #333333;
                border: 1px solid #4a4a4a;
            ...
            QGroupBox#CategoryManagerBox QPushButton:hover, QGroupBox#SecondaryScriptsBox QPushButton:hover {
                background: #3d3d3d;
                border-color: #5a5a5a;
            }
            QGroupBox#CategoryManagerBox QPushButton[op="add"], QGroupBox#SecondaryScriptsBox QPushButton[op="add"] {
                color: #f4f4f4;
                border: 1px solid #ff9f2e;
                background: #303030;
            }
            QGroupBox#CategoryManagerBox QPushButton[op="add"]:hover, QGroupBox#SecondaryScriptsBox QPushButton[op="add"]:hover {
                background: #39332b;
                border-color: #ffad3b;
            }
    ...
    def log_layout_state(self, reason=""):
        ...
        try:
            ...
            if mode == "Horizontal":
                options_rect = _elk_debug_rect(self.h_options_btn) if hasattr(self, "h_options_btn") else "n/a"
                search_rect = _elk_debug_rect(self.h_search_btn) if hasattr(self, "h_search_btn") else "n/a"
                add_rect = _elk_debug_rect(self.h_add_btn) if hasattr(self, "h_add_btn") else "n/a"
                avail_h = self.scroll.viewport().height() if hasattr(self, "scroll") and self.scroll is not None else self.height()
                computed = getattr(self, "_last_horizontal_control_size", "n/a")
                overlap = False
                try:
                    rects = [b.geometry() for b in (self.h_options_btn, self.h_search_btn, self.h_add_btn) if b is not None]
