...
def _indirect_actions_from_kmi(kmi) -> list[str]:
    """Extract indirectly-triggered actions/tools from operator properties.

    Some shortcuts do not map directly to the final action label shown in keymaps.
    Example: tool cycling operators can trigger a specific tool (e.g. Bevel) even
    when the direct keymap label is generic ("Set Tool by Name").
    """
    actions = []

    # Known tool-selection operators often hide the actual target in properties.
    if kmi.idname in {"wm.tool_set_by_id", "wm.tool_set_by_name", "wm.tool_set_by_index"}:
        prop_candidates = []
        for attr in ("name", "idname", "tool", "tool_name"):
            value = getattr(kmi.properties, attr, None)
    ...
    # Generic fallback: extract readable string properties that may encode target
    # actions/tools in custom add-ons or shortcut-cycle implementations.
    for prop_name in kmi.properties.bl_rna.properties.keys():
        if prop_name == "rna_type":
            continue
        value = getattr(kmi.properties, prop_name, None)
