...
"""
    ...
    def _build_guided_integration_review_section(self) -> None:
        ...
        cmds.button(label="Add Entry (Main Assets)", height=UI_BUTTON_HEIGHT, command=lambda *_: self.add_integration_catalog_asset("main"))
        ...
        cmds.button(label="Add Entry (Annexe)", height=UI_BUTTON_HEIGHT, command=lambda *_: self.add_integration_catalog_asset("annexe"))
    ...
    def add_integration_catalog_asset(self, asset_type: str) -> None:
        is_main = asset_type == "main"
        list_key = "integration_catalog_list" if is_main else "integration_annexe_catalog_list"
        result = cmds.promptDialog(
            title="Add Asset Entry",
            message="Asset name:",
            button=["Add", "Cancel"],
            defaultButton="Add",
            cancelButton="Cancel",
        ...
        if not new_name:
            return
        target_list = self.integration_main_catalog_assets if is_main else self.integration_annexe_catalog_assets
        if new_name not in target_list:
            target_list.append(new_name)
        if is_main:
            self.integration_main_catalog_assets = sorted(set(target_list))
        else:
            self.integration_annexe_catalog_assets = sorted(set(target_list))
        self.integration_detection_sources.setdefault(new_name, set())
        if not is_main:
            self.integration_annexe_sources.setdefault(new_name, set())
        self._integration_rebuild_catalog_assets()
        self._refresh_integration_catalog_list_ui()
        self._set_selected_list_control_items(list_key, [new_name])
        self._append_integration_log(f"[USER EDIT] Added {'main' if is_main else 'annexe'} asset: {new_name}")
