...
try:
    import maya.OpenMayaUI as omui  # type: ignore[assignment]
except Exception:
    omui = None

try:
    from PySide2 import QtCore, QtGui, QtWidgets  # type: ignore[assignment]
    from shiboken2 import wrapInstance  # type: ignore[assignment]
    QT_BINDING = "PySide2"
except Exception:
    try:
        # Maya 2025+ can expose Qt6 only in some environments.
        from PySide6 import QtCore, QtGui, QtWidgets  # type: ignore[assignment]
        from shiboken6 import wrapInstance  # type: ignore[assignment]
        QT_BINDING = "PySide6"
    except Exception:
...
if QtWidgets is not None:
    ...
    StepBadgeWidget = None
    OutsourceReviewWindow = None


def _qt_build(self: HighPolyReviewTool) -> None:
    if QtWidgets is None:
        raise RuntimeError("Qt indisponible (PySide2/PySide6). Impossible de construire l'interface principale.")
