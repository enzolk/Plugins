...
class HighPolyReviewTool:
    ...
    def _append_detailed_log_row(
        ...
        if row_color:
            button_kwargs["backgroundColor"] = row_color
            button_kwargs["enableBackground"] = True
        else:
            button_kwargs["enableBackground"] = False
        row_control = cmds.button(**button_kwargs)

        if row_has_objects:
            self.result_control_to_objects[row_control] = list(objects or [])
